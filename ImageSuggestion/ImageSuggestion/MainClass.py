...
class MainClass:
    ...
    def readfile(self,imputfilename):
        ...
        try:
            with codecs.open(imputfilename,"r",encoding=encode) as file:
                self.rawText = Text(file.read().decode(encode))
    ...
    def splitSentence(self):
        ...
        for section in self.sectionList[:]:
            if(section.text.endswith(u"。")):
                matchIter = pre.finditer(section.text)
                section.sentenceList = [Text(t.group()) for t in matchIter]
            else:
    ...
    def writefile(self,outputfilename):
        with codecs.open(outputfilename,"w",encoding="utf-8-sig") as file:
            for section in self.defficultySortedSectionList:
                file.write((unicode(main.sectionList.index(section)) + section.text\
